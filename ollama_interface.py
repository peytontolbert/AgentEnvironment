...
class OllamaInterface:
    ...
    async def collect_feedback(self) -> Dict[str, Any]:
        ...
        """
        prompt = "Collect feedback from the current project and system state."
        context = {"project_state": self.knowledge_base.get_project_state()}
        response = await self.query_ollama(self.system_prompt, prompt, context=context)
        if response:
            self.logger.info(f"Collected feedback: {response}")
            return response
        else:
            self.logger.error("No feedback collected from Ollama")
